...
1. **Provide an admin username with password**
* Create an environment variable with a username (you will use it for the HTTP Basic Auth for Registry), for example:
...
* Set these environment variables, right before the next command:
  * `DOMAIN=registry.tmd.fakhred.in`
  * `TRAEFIK_TAG=registry.com`
